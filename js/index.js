...
$(".color_container").each(function() {
	const sectionName = $(this).parent(".team_settings").attr("class").split(" ").pop(); //lowercase team name [home]
	...
	$(this).find(".color_dot").click(function() {
	...
	$(this).append(`<form id="${sectionID}"><ul class="colorPanel color_list"></ul></form>`);
	const container = $(`#${sectionID}`);
	$.each(colorsOptions, function(i, e) {});
	var cList = $(this).find(".color_list");
	$.each(colorsOptions, function(i, e) {
		var li = $("<li/>").appendTo(cList);
		var inputs = $("<input/>").attr("type", "radio").attr("id", e.colorName + team).attr("name", "color" + team).attr("value", e.colorName)
			// .text(e.name)
			.appendTo(li);
		var labels = $("<label/>").attr("for", e.colorName + team).appendTo(li);
	...
	function setNewColorValue() {
		...
		$(`.color_container.${sectionName}`).find(".color_dot").removeClass().addClass("color_dot").addClass(selectedColor);
		$(`#team${team}`).removeClass().addClass(`score ${sectionName}`).addClass(selectedColor);
	}
